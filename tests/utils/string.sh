...
assert.equal 'hi' "$(string nth-token 0 hi there)"
assert.equal 'there' "$(string nth-token 1 hi there)"
assert.equal 'there' "$(string nth-token -1 hi there)"
assert.equal 'hi' "$(string nth-token -2 hi there)"
assert.false string nth-token -3 hi there
assert.false string nth-token 2 hi there

assert.equal 'hi' "$(IFS=: string nth-token 0 "hi:there folks")"
assert.equal 'there folks' "$(IFS=: string nth-token 1 "hi:there folks")"
...
assert.equal $'ok\nalright' "$(string filter-not 'okay' $'ok\nokay\nalright')"
assert.equal 'alright' "$(string filter-not 'ok' $'ok\nokay\nalright')"
assert.equal 'alright then' "$(echo $'ok\nokay\nalright then' | string filter-not 'ok')"
