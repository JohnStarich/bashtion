...
assert true retrier retry echo hi >/dev/null
...
assert true ! retrier retry bad_exit_code
...
fail_counter=1
assert true retrier retry succeed_nth_time 0

fail_counter=1
assert true retrier retry succeed_nth_time 1

fail_counter=1
assert false retrier retry succeed_nth_time 10

fail_counter=1
assert true retrier retry 10 succeed_nth_time 10
